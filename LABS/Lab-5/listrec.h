...
// PART2: CS210 Lab 5_Prints in reverse
template < class DT >
...
// PART2: insert the character 'a' immediately before each occurence of the character 'b'.
...
{
  if (p != 0) //if p is not empty then move p until the end of node
  {
    aBeforebSub(p->next);
    if (p->dataItem == 'b') //if b if found
    {
      p = new ListNode<DT>('a',p); //insert a before b
    }
  }
} 