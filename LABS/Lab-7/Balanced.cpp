...
int main()
{
  ...
  //try clause added where an exception might occur
  try{
    while (symbol != '\n' && balanced)
      {
      if (IsOpen(symbol))
    ...
    //if it balance and empty stack, output well formed expression
    ...
    //catch clause to catch those FullStack exception!
    catch(FullStack)
    {
